...
Sources
  PubMed          the Nature and Science families in data/vocab.yaml (default);
                  --include-broad-journals adds Cell, NEJM, Lancet, JAMA, PNAS
  bioRxiv/medRxiv opt-in via --include-preprints
  arXiv           opt-in via --include-arxiv

This script calls no paid API. Domain routing is a convenience that uses
ANTHROPIC_API_KEY when one happens to be set; without it the scan is identical
and candidates simply arrive unrouted, which is what `/weekly-scan` expects --
it does the routing itself inside your Claude Code session, at no extra cost.
...
def build_pubmed_query(
    vocab: Vocab, since: date, until: date, broad: bool = False
) -> str:
    journals = [
        meta["pubmed"]
        for meta in vocab.venues.values()
        if meta.get("pubmed") and (broad or meta.get("high_impact"))
    ]
...
# ---------------------------------------------------------------------------
# Domain routing via the Anthropic API (optional, and billed)
#
# Nobody needs to set this up. It exists for an unattended CI run; the normal
# path is `/weekly-scan`, where the agent reading this output routes the
# candidates itself on your existing subscription.
# ---------------------------------------------------------------------------
...
def main() -> int:
    ...
    parser.add_argument(
        "--include-broad-journals",
        action="store_true",
        help="also scan the journals marked high_impact: false (Cell, NEJM, Lancet, "
        "JAMA, PNAS, JCO). The default is the Nature and Science families only",
    )
    parser.add_argument(
        "--check-journals",
        action="store_true",
    ...
    sources: list[tuple[str, object]] = [
        ("pubmed", lambda: fetch_pubmed(vocab, since, until, broad=args.include_broad_journals)),
    ]
